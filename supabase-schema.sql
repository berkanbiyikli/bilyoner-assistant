  ...
  CREATE INDEX IF NOT EXISTS idx_predictions_date ON predictions(kickoff);
  ...
  CREATE INDEX IF NOT EXISTS idx_bankroll_user ON bankroll_entries(user_id);
  CREATE INDEX IF NOT EXISTS idx_bankroll_date ON bankroll_entries(created_at);
  ...
  CREATE INDEX IF NOT EXISTS idx_tweets_type ON tweets(type);
  ...
  CREATE INDEX IF NOT EXISTS idx_odds_date ON odds_snapshots(kickoff);
  ...
  DROP POLICY IF EXISTS bankroll_select ON bankroll_entries;
  CREATE POLICY bankroll_select ON bankroll_entries FOR SELECT USING (auth.uid() = user_id);
  DROP POLICY IF EXISTS bankroll_insert ON bankroll_entries;
  ...
  DROP POLICY IF EXISTS odds_snapshots_select ON odds_snapshots;
  CREATE POLICY odds_snapshots_select ON odds_snapshots FOR SELECT USING (true);
  DROP POLICY IF EXISTS odds_snapshots_insert ON odds_snapshots;
  ...
  DROP POLICY IF EXISTS validation_select ON validation_records;
  CREATE POLICY validation_select ON validation_records FOR SELECT USING (true);
  DROP POLICY IF EXISTS validation_insert ON validation_records;
  CREATE POLICY validation_insert ON validation_records FOR INSERT WITH CHECK (true);
  DROP POLICY IF EXISTS validation_update ON validation_records;
